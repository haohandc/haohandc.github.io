...
description: 去年比赛的WP，最早期写的，可能有点问题。
categories: [WP, "2025"]
tags: [WP]
---
...
很简单，一张图片![](assets/img/media//2026_4_2/location.jpg)里面有位置信息，轻轻松松找到flag：`flag{1_4_HuiZhanZhongXin}`
...
“有这张照片，问题就解决了。”![](assets/img/media//2026_4_2/screenshot.jpg)
...
![](assets/img/media//2026_4_2/Pasted%20image%2020251008144435.png)
找到第三关
![](assets/img/media//2026_4_2/屏幕截图%202025-10-04%20214752.png)
很明显一个base64字符串
![](assets/img/media//2026_4_2/屏幕截图%202025-10-04%20211908.png)
但是解码后发现有问题，提示我们去玩一下游戏，按照提示弄一下就能看到 ~~（别告诉我你没过新手教程而不会操作）~~ 
![](assets/img/media//2026_4_2/屏幕截图%202025-10-04%20213003.png)
实际的密文应该是这样的`LzkuL4gSqI6hrH0JYTUkLR9jLy0PnQNlKIY1Y4VmJ2DlqFZso4VsDzQkpmYusC==`
替换的字符在上面
![](assets/img/media//2026_4_2/Pasted%20image%2020251008144515.png)
...
过程：我们下载下来，解压发现需要密码。属性一栏能看到`c5im???`，我们打开ARCHPR进行掩码爆破，![](assets/img/media//2026_4_2/Pasted%20image%2020250928195015.png)![](assets/img/media//2026_4_2/Pasted%20image%2020250928195017.png)
...
另一个在word文件夹里，
![](assets/img/media//2026_4_2/Pasted%20image%2020250928195345.png)
...
过程：分析流量数据，用**Wire Shark**，打开来看，![](assets/img/media//2026_4_2/Pasted%20image%2020250928195734.png)
![](assets/img/media//2026_4_2/Pasted%20image%2020250928195813.png)
一眼看到`flag.zip`和`password.txt`，我们导出保存一下![](assets/img/media//2026_4_2/Pasted%20image%2020250928195917.png)
...
常见的隐藏手法是修改图片的长宽，这里我们打开010修改下宽度![](assets/img/media//2026_4_2/Pasted%20image%2020250928200253.png)
宽度是第二行的5-8字节存储的数据，我们稍微改大一点![](assets/img/media//2026_4_2/Pasted%20image%2020250928200410.png)
得到关键信息。我们打开stegsolve，重新弄个没修改过的图片打开（修改过不行）
![](assets/img/media//2026_4_2/Pasted%20image%2020250928200644.png)
![](assets/img/media//2026_4_2/Pasted%20image%2020250928200712.png)
...
过程：题目提示这里用到了维吉尼亚加密。我们来看这个wav的频谱图，能看到![](assets/img/media//2026_4_2/Pasted%20image%2020250928200942.png)
`KEY{deepsound}`。去搜一下，能找到软件deepsound，下下来把wav拖进去，看到
![](assets/img/media//2026_4_2/Pasted%20image%2020250928201133.png)
...
![](assets/img/media//2026_4_2/Pasted%20image%2020251015172844.png)
...
过程：我们打开容器，游戏不玩，F12看一眼![](assets/img/media//2026_4_2/Pasted%20image%2020250928201655.png)
...
过程：我们根据提示，用dnspy打开来看![](assets/img/media//2026_4_2/Pasted%20image%2020250928203755.png)
我们能找到密文`D1ucj0u!tqjwf!fohjoffsjoh!xj!epspqz!ju!gvo!2025`。下面的program里是加密函数
![](assets/img/media//2026_4_2/Pasted%20image%2020250928204733.png)
我们照着写一写，得到脚本
...
![](assets/img/media//2026_4_2/Pasted%20image%2020250928205138.png)
就能写出脚本（逆向操作一下就完事了）
...
题干：觉得Main很史可以直接去看加密
过程：安卓逆向，jadx看一下
![](assets/img/media//2026_4_2/Pasted%20image%2020250928205409.png)
找到一个加密工具，
...
![](assets/img/media//2026_4_2/Pasted%20image%2020250928210435.png)
找到了srand的种子`0x3039`。由于这是一个linux的可执行文件，rand在相同种子情况下生成的随机数不同。
看主函数，flag长度位27，最后比较部分看着有点奇怪，双击看过去
![](assets/img/media//2026_4_2/Pasted%20image%2020250928210830.png)
...
linux环境下编译运行一下，
![](assets/img/media//2026_4_2/9b14506e616064af20a4ae0124c06dc4.png)
发现得到的flag似乎有点奇怪（实际上已经能自己拼出正确的flag了）~~不是为啥啊，为啥会这样~~
多跑几遍
![](assets/img/media//2026_4_2/0f6113de340c25ffdf9066dc20e3c7af.png)
...
![](assets/img/media//2026_4_2/Pasted%20image%2020251006005516.png)
头几个就是我们要的东西
...
题干：这个exe怎么看起来有点不一样...
过程：这是个python逆向题，我们有一个`pyc.exe`，![](assets/img/media//2026_4_2/Pasted%20image%2020251006003830.png)
...
如果python不是3.8版本的![](assets/img/media//2026_4_2/Pasted%20image%2020251006004139.png)
会提示版本不对，并且跳过解压过程。我们下个3.8的python，并直接到对应目录下重新执行以上命令![](assets/img/media//2026_4_2/Pasted%20image%2020251006004240.png)
就能得到解压后的东西找一下里面的`pyc.pyc`，这里是主程序
![](assets/img/media//2026_4_2/Pasted%20image%2020251006004336.png)
...
DIE能识别出UPX![](assets/img/media//2026_4_2/Pasted%20image%2020251006001144.png)
我们用x64dbg来手动脱壳，使用ESP(64位系统对应RSP)定律法来手动脱壳。[参考文献](https://www.cnblogs.com/Meteor-Kai/articles/18473549)
先按F9跳到程序提供的入口点
![](assets/img/media//2026_4_2/Pasted%20image%2020251006001402.png)
这里4个push相当于pushad，我们在这之后下个断点，运行到这里，能看到RSP被修改了
![](assets/img/media//2026_4_2/Pasted%20image%2020251006001519.png)
这之后，我们对栈顶的内存地址(RSP存储的地址)下个断点（在转储中跟随）（大小随意）
![](assets/img/media//2026_4_2/Pasted%20image%2020251006002933.png)
![](assets/img/media//2026_4_2/Pasted%20image%2020251006002935.png)
下完后，按F9运行，肯定能看到pop或者popad
![](assets/img/media//2026_4_2/Pasted%20image%2020251006003111.png)
这下面有一个大跳，我们双击跳转过去，跳到的地方就是OEP（原始入口点）了
然后用插件dump（转储）一下（断点下不下无所谓，可以下一个防止手贱）
![](assets/img/media//2026_4_2/Pasted%20image%2020251006003314.png)
...
![](assets/img/media//2026_4_2/Pasted%20image%2020251006014630.png)
得到带空格的`f7 5f 7a c1 5d 34 db d6 2f d8 75 2d de e1 da 68 e0 57 9b 4a ce ea 07 f9 5e 79 5e`
方便我们之后修改数据。之后我们下断点，随便输点东西
![](assets/img/media//2026_4_2/Pasted%20image%2020251006014854.png)
双击data跳转，然后patch bytes修改，先直接粘贴整个
![](assets/img/media//2026_4_2/Pasted%20image%2020251006014932.png)
再比对一下，把剩下的`e0 57 9b 4a ce ea 07 f9 5e 79 5e`复制进去
![](assets/img/media//2026_4_2/Pasted%20image%2020251006015044.png)
之后回到伪代码，F8单步运行一下，`rc4_crypt()`就帮我们解密完了。再双击data跳转一下，我们看到了flag
![](assets/img/media//2026_4_2/Pasted%20image%2020251006014358.png)
我们按A识别为文字
![](assets/img/media//2026_4_2/Pasted%20image%2020251006014401.png)
...
![](assets/img/media//2026_4_2/Pasted%20image%2020251005235704.png)
...
{% raw %}
{% comment %} print(f"flag{{{reversed_hex}}}") {% endcomment %}
{% endraw %}
```
...
看一眼函数表，
![](assets/img/media//2026_4_2/Pasted%20image%2020251006171724.png)
很快发现主函数和解密函数。主函数是
...
用mazesolver看看（已打通时）
![](assets/img/media//2026_4_2/Pasted%20image%2020251018131000.png)
未打通时
![](assets/img/media//2026_4_2/屏幕截图%202025-10-18%20112808.png)
我们用mazesolver多次更改起点终点（分别到4，5，6三个墙前头），每个墙推几下
到4  ssddddwwa  然后推一下  a
![](assets/img/media//2026_4_2/Pasted%20image%2020251018131140.png)
到5  dddssssddwwddssssaassddddddwwwwaawww  推两下 ww
![](assets/img/media//2026_4_2/Pasted%20image%2020251018131205.png)
到6  sssddssssssaaaassaawwaawwaaaassddssddssaaaawwaawww  推三下 www
![](assets/img/media//2026_4_2/Pasted%20image%2020251018131225.png)
组合起来是这个
